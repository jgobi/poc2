...
export class SiQADFile {
  ...
  async runSimulation(
    ...
    let results = parseXML(file).sim_out.elec_dist.dist || []; // Sometimes there are no results
    if (!Array.isArray(results)) results = [results]; // Sometimes there is only one result
